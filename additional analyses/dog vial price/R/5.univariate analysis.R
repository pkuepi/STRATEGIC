...
decision_tree_univ_draw<-function(vary,
  ...
  if(DogVax_TF==T){
    cost_dog_per_year = c(dog_vial_price*6.8996*0.4*dogpop[1] * exp(-discount*future[1]),
                          dog_vial_price*6.8996*0.7*dogpop[2:horizon] * exp(-discount*future[2:horizon]))
    }else{
    cost_dog_per_year = dog_vial_price*6.8996*0.4*dogpop[1:horizon] * exp(-discount*future)
  }
  if(IBCM == F){
